...
class Ls_Balance_Base(SQLModel):
    """LS 증권 계좌 잔고 정보 기본 모델"""
...
class Ls_Balance(Ls_Balance_Base, table=True):
    """LS 증권 계좌 잔고 정보 테이블"""
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True, description="잔고 정보 ID")
    account: "Account" = Relationship(back_populates="ls_balances")

    class Config:
        table_name = "ls_balances"
        description = "LS 증권 계좌 잔고 정보 테이블"

    __table_args__ = (
        Index('ix_ls_balances_account_timestamp', 'account_id', 'timestamp'),
    )

class Ls_Trade_Base(SQLModel):
    """LS 증권 거래내역 기본 모델"""
    account_id: uuid.UUID = Field(foreign_key="account.id", description="계좌 ID")
...
class Ls_Trade(Ls_Trade_Base, table=True):
    """LS 증권 거래내역 테이블"""
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True, description="거래내역 ID")
    account: "Account" = Relationship(back_populates="ls_trades")

    class Config:
        table_name = "ls_trades"
        description = "LS 증권 거래내역 테이블"

    __table_args__ = (
        Index('ix_ls_trades_account_date', 'account_id', 'trade_date'),
        Index('ix_ls_trades_stock', 'account_id', 'stock_code'),
    )

class Ls_Daily_Trade_Base(SQLModel):
    """LS 증권 일별 거래내역 기본 모델"""
    # output1 필드
...
class Ls_Daily_Trade(Ls_Daily_Trade_Base, table=True):
    """LS 증권 일별 거래내역 테이블"""
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    account_id: uuid.UUID = Field(foreign_key="account.id", nullable=False)
    account: Optional["Account"] = Relationship(back_populates="ls_daily_trades")
    ...
    class Config:
        table_name = "ls_daily_trades"
        description = "LS 증권 일별 거래내역 테이블"

    __table_args__ = (
        UniqueConstraint('account_id', 'order_date', 'order_no', name='uix_ls_daily_trade_account_order'),
        Index('ix_ls_daily_trades_account_date', 'account_id', 'order_date'),
        Index('ix_ls_daily_trades_stock', 'account_id', 'stock_code'),
    )

class Ls_Minutely_Balance(SQLModel, table=True):
    """LS 증권 분별 잔고 정보 테이블"""
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    account_id: uuid.UUID = Field(foreign_key="account.id", nullable=False)
    account: Optional["Account"] = Relationship(back_populates="ls_minutely_balances")
    ...
    class Config:
        table_name = "ls_minutely_balances"
        description = "LS 증권 분별 잔고 정보 테이블"

    __table_args__ = (
        Index('ix_ls_minutely_balances_account_timestamp', 'account_id', 'timestamp'),
    )

class Ls_Daily_Trade_Response(SQLModel):
    """LS 증권 일별 거래내역 응답 모델"""
    message: str
    updated_count: int
    start_date: str
...
class Ls_Balance_Response(SQLModel):
    """LS 증권 잔고 조회 API 응답 모델"""
    output1: List[Dict[str, Any]] = Field(default_factory=list, description="보유종목 정보")
    output2: List[Dict[str, Any]] = Field(default_factory=list, description="계좌 잔고 정보")
    rsp_cd: str = Field(..., description="응답 코드")
    rsp_msg: str = Field(..., description="응답 메시지")
